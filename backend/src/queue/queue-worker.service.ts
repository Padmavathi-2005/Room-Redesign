...
export class QueueWorkerService implements OnApplicationBootstrap {
  ...
  private async executeJob(room: RoomDocument) {
    ...
    try {
      ...
      console.log(`📝 [PROMPT BUILDER] 12-Stage Prompt Engine compiled structural constraints successfully.`);
      console.log(`   - Compiled Prompt (truncated): "${finalPrompt.slice(0, 150)}..."`);

      // 3. Dispatch image generation request to ProviderManager
      this.logger.log(`Step 2: Dispatching job payload to provider manager...`);
      
      const isManusConfigured = !!(process.env.MANUS_API_KEY && process.env.MANUS_API_KEY.trim() !== '');
      ...
      const downloadResponse = await axios.get(generationResult.imageUrl, { responseType: 'arraybuffer' });
      const generatedBuffer = Buffer.from(downloadResponse.data);
      const generatedMimeType = String(downloadResponse.headers['content-type'] || 'image/jpeg');
